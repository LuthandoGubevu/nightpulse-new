...
export function ClubForm({ club, mode }: ClubFormProps) {
  ...
  const defaultValues: Partial<ClubFormValues> = club
    ? {
        ...
        currentCount: club.currentCount ?? 0,
        thresholdLow: club.capacityThresholds?.low ?? 0,
        thresholdModerate: club.capacityThresholds?.moderate ?? 0,
        thresholdPacked: club.capacityThresholds?.packed ?? 0,
